...
if [ ! -z "$ADMIN_CDN_EXISTS" ]; then
  ...
  aws s3 cp public/css/link.min.css s3://assets.us.auth0.com/extensions/$EXTENSION_NAME/$CURRENT_VERSION/admin.min.css --region us-west-1 --quiet --cache-control "max-age=86400" --acl public-read
  echo "$CURRENT_VERSION uploaded to the cdn"
fi
