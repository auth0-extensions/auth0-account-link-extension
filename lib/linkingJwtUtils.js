...
const fetchRegisteredCustomDomains = async (customDomain) => {
...
const configureIssuer = async () => {
  ...
  if (customDomain) {
    try {
      const registeredCustomDomain = await fetchRegisteredCustomDomains(customDomain);
      return qualifyDomain(registeredCustomDomain);
    } catch (error) {
      logger.error('Error fetching custom domain: ', error);
      throw new Error(`Error fetching custom domain: ${error.message}`);
    }
  }

  return qualifyDomain(config('AUTH0_DOMAIN'));
};
